...
void scene::GeometryShaderScene::DoPreviousInit()
{
	...
	m_ExplosionShader = ShaderProgram("res/shaders/GeometryExplosionShader.shader");
	m_ExplosionShader.SetUniformBlockBinding("Matrices", 0);

	m_NormalShader = ShaderProgram("res/shaders/GeometryNormalShader.shader");
	m_NormalShader.SetUniformBlockBinding("Matrices", 0);

	m_Camera = Camera(glm::vec3(0, 0, 10), glm::vec3(0.0f, 1.0f, 0.0f), -90.0f, 0.0f);
	m_Camera.Fov = 80;
}
