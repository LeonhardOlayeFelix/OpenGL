...
scene::AsteroidScene::AsteroidScene()
{
	...
	m_Shader = ShaderProgram("res/shaders/AsteroidScenePlanetShader.shader"); 
	m_Shader2 = ShaderProgram("res/shaders/AsteroidSceneRockShader.shader"); 
	m_Planet = Model("res/models/planet/planet.obj");
	m_Rock   = Model("res/models/rock/rock.obj"); 

	//This will store the per instance data needed to transform each asteroid in a buffer on the GPU
	m_InstanceVBO = VertexBuffer(&m_ModelMatrices[0], sizeof(m_ModelMatrices));
...
{
	m_Camera = Camera(glm::vec3(0.0f, 100.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 0.0f, -90.0f);
	m_Camera.Fov = 80;
}
